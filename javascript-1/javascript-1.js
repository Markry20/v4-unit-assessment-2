...
for (let i=0; i<lettersToPair.length;i++){
    for (let m = i + 1; m<lettersToPair.length;m++){
        if(lettersToPair[i] === lettersToPair[m]){
            pairsArray.push([i,m])
        }
    }
}
...
//CODE HERE
  let phone1 = new Phone('Iphone','Pro',256,'yellow',false);
  
  let phone2 = new Phone('Android','Samsung',64,'black',false);
  
  let phone3 = new Phone('Iphone','Pro',144,'blue',false);
